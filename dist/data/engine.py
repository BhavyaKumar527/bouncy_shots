...
class particle(object):
    ...
    def draw(self,surface,scroll):
        ...
        if self.color == None:
            img = particle_images[self.type][int(self.frame)]
        else:
            img = swap_color(particle_images[self.type][int(self.frame)],(0,0,0),self.color)
        surface.blit(img,(self.x-scroll[0]-int(img.get_width()/2),self.y-scroll[1]-int(img.get_height()/2)))
    def update(self):
